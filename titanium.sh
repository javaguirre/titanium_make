...
TI_ASSETS_DIR="${TI_DIR}/mobilesdk/linux/${TI_SDK_VERSION}" # Both iOS and Android SDKs are linked in this directory
...
APP_ID=`cat ${PROJECT_ROOT}/${PROJECT_NAME}/tiapp.xml | grep "<id>" | sed -e "s/<\/*id>//g"`
APP_NAME=`cat ${PROJECT_ROOT}/${PROJECT_NAME}/tiapp.xml | grep "<name>" | sed -e "s/<\/*name>//g"`
...
if [ ${APP_DEVICE} == "iphone" ] || [ ${APP_DEVICE} == "ipad" ]; then
	...
	bash -c "python2 ${TI_IPHONE_BUILD} run ${PROJECT_ROOT}/${PROJECT_NAME}/ ${IPHONE_SDK_VERSION} ${APP_ID} ${APP_NAME} ${APP_DEVICE}"
